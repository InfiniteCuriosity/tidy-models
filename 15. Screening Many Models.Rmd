...
Packages <- c("tidymodels", "finetune", "rules", "baguette", "ggrepel", "xgboost", "kknn", "glmnet")
...
The `compressive_strength` column is the outcome. The `age` predictor tells us the age of the concrete sample at testing in days (concrete strengthens over time) and the rest of the predictors like `cement` and `water` are components in units of kilograms per cubic meter.
...
```{r Use **parsnip** to create a set of model specifications}
...
```{r Extract the parameter set, to have the correct parameter range}
...
```{r Combine the recipe that only standardized the predictosr to the nonlinear models}

normalized <- 
  workflow_set(
    preproc = list(normalized = normalized_recipe),
    models = list(SVM_radial = svm_r_spec, SVM_poly = svm_p_spec, KNN = knn_spec, neural_network = nnet_spec)
  )
normalized
```
...
```{r Add the neural network parameter object to the workflow}
...
When a function from the **tune** or **finetune** package is used to tune (or resample) the workflow, this argument (`option`) will be used.
...
```{r Create another workflow set fr the outcome and predictors}
...
no_pre_proc <- 
  workflow_set(
    ...
    models = list(MARS = mars_spec, CART = cart_spec, CART_bagged = bag_cart_spec, RF = rf_spec, boosting = xgb_spec, Cubist = cubist_spec)
  )
...
Finally, the test that uses nonlinear terms and interactions with the appropriate models is assembled:

```{r Assemble set of non-linear terms}
...
all_workflows <- 
  ...
  # Make the workflow IDs a little simpler
...
```{r Showing that `option` and `result` columns have been updated}
...
full_results_time <- 
  system.time(
    grid_results <- 
      all_workflows %>% 
      workflow_map(seed =1503, resamples = concrete_folds, grid = 25, control = grid_ctrl, verbose = TRUE)
  )
...
the `option` column now contains all of the options we used in the `workflow_map()` call. This makes our results reproducible. In the `result` columns, the "`tune[+]`" and "`rsmp[+]`" notations mean that the object had no issues. A value such as "`tune[x]" occurs if all of the models failed for some reason.
...
```{r Filter to look only at RMSE}

grid_results %>% 
  rank_results() %>% 
  filter(.metric == "rmse") %>% 
  select(model, .confit, rmse = mean, rank)

```

Also by default, the function ranks all the candidate sets; that's why the same model can show up multiple times in the output. An option, called `select_best`, can be used to rank the models using their best tuning parameter combination.
...
```{r Plotting best results for each model}
...
autoplot(
  ...
  geom_test(aes(y = mean - 1/1, label = wflow_id), angle = 90, hjust = 0.5) +
  lims(y = c(3, 9.5)) +
...
```{r Plotting the tuning parameters for a specific model}
...
The example model screening with our concrete mixture data firs a total of 25,200 models. Using two workers in parallel, the estimation took the authors 2.5 hours to complete.
...
```{r Looking at race results}

race_results

```

The same helpful functions are available for this object to interrogate the results and, in fact, the basic `autoplot()` method shown in Figure $15.3^30$ produces trends similar to Figure 15.2. This is produced by:

```{r Autoplot race results}
...
Overall, the racing approach estimated a total of 4,652 models, 18.46% of the full set of 25,200 models in the full grid. As a result, the racing approach was 4.3-fold faster.
...
```{r Comparing racing results with full results}
...
matched_results <- 
  ...
  inner_join(
    rank_results(grid_results, select_best = TRUE) %>% 
      select(wflow_id, .metric, complete = mean, config_comlete = .config, model),
...
```{r See the test metric results}

collect_metrics(boostng_test_results)

```

```{r Visualize the test results}
...
We see here how well the observed and predicted compressive strength for these concrete mixtures align. (very well!)
